    ...
    class clsUtilities
    {
        ...
        { /* Testdaten:
            */
            ...
            DateTime time; // Versuch, das Datum direkt zu konvertieren
            string[] formats = { "H:m",
                                 "H:m:s",
                                 "H.m",
                                 "H.m.s",
            ...
            if (DateTime.TryParseExact(timeString, formats, new CultureInfo("de-DE"), DateTimeStyles.None, out time)) { return time; }
            int hour = 0, minute = 0;
            string pattern = @"^(?:" +
        ...
        { /* Testdaten:
           ...
           */
            ...
            DateTime date; // Versuch, das Datum direkt zu konvertieren
            ...
            if (DateTime.TryParseExact(dateString, formats, new CultureInfo("de-DE"), DateTimeStyles.None, out date)) { return date; }
        ...
        public static void removeEmptyRows(DataGridView gridView, List<int> intDTEdits)
        {
            for (int row = 0; row < gridView.RowCount; row++)
            {
                ...
                {
                    if (intDTEdits.Contains(col) && gridView.Rows[row].Cells[col].Value != null && !isNullOrWhiteSpace(gridView.Rows[row].Cells[col].Value.ToString()))
                    { isEmpty = false; break; }
                }
